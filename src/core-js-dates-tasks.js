...
function getNextFriday(date) {
  ...
  let plusDays = (5 - currentDay + 7) % 7;
  if (plusDays === 0) {
    plusDays = 7;
  }
  const nextFriday = new Date(currentDate);
  nextFriday.setDate(currentDate.getDate() + plusDays);
  return nextFriday;
}
...
function getCountDaysInMonth(month, year) {
  ...
  const daysOfTheLastMonth = new Date(year, realMonth - 1, 0);
  const days = daysOfTheLastMonth.getDate();
  return days;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const obeDayMs = 86400000;
  const days = difference / obeDayMs + 1;
  return days;
}
...
function formatDate(date) {
  ...
  const hours = dateToFormat.getUTCHours();
  ...
  const resultHours = hours >= 12 ? 'PM' : 'AM';
  let formattedHours;
  if (hours % 12 === 0) {
    formattedHours = 12;
  } else {
  ...
  const formattedDate = `${month}/${day}/${year}, ${formattedHours}:${formattedMinutes}:${formattedSeconds} ${resultHours}`;
...
function getWeekNumberByDate(date) {
  ...
  const dayOfTheWeek = dateNow.getDay();
  const indexDayOfTheWeek = dayOfTheWeek === 0 ? 6 : dayOfTheWeek - 1;
  const calendarThursday = new Date(dateNow);
  calendarThursday.setDate(dateNow.getDate() - indexDayOfTheWeek + 3);
  const firstDayOfTheYear = new Date(calendarThursday.getFullYear(), 0, 1);
  const difference = calendarThursday.getTime() - firstDayOfTheYear.getTime();
  const weekNumber = Math.floor(difference / (1000 * 3600 * 24) / 7) + 1;
  return weekNumber;
}
...
function isLeapYear(date) {
  ...
  const is29February = new Date(year, 1, 29);
  return is29February.getDate() === 29;
}
